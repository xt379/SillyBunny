...
Fairy 是 [Fairy](https://github.com/SillyTavern/SillyTavern) 的精致分支，其简洁的图形化外壳界面同时适配桌面端和移动端，设计灵感来自 [GNOME 项目](https://www.gnome.org/)和 [KDE Plasma](https://kde.org/plasma-desktop/)。Fairy 采用基于 Bun 的运行时以提升性能；提供可快速访问的 Home 页面，内含教程、指南与推荐扩展；搭载轻量级聊天内智能体系统，以支持现代智能体功能；还提供更多聊天模式，拓展角色卡的使用方式，并包含大量错误修复与常规改进！

> [!WARNING]
> 我们是一个仅由三人组成的小团队，热衷于打造简洁高效的前端，在利用 Fairy 出色后端成果的同时，实现我们一直希望在其中看到的所有功能。
>
> 因此，本项目仍处于开发阶段，目前仍仅具备 Beta 版质量。[请将 Fairy 特有的问题提交到本项目的 issue tracker。](https://github.com/SillyBunnyTeam/SillyBunny/issues) 如果问题也能在上游 Fairy 中复现，请改为向上游报告。
>
...
bun run start:global   # 使用 Fairy 管理的数据路径
...
- 只需运行一次 `termux-setup-storage`，即可授权 Fairy 访问共享文件；请勿将仓库克隆到共享存储中
...
Git 检出目录既可通过启动器更新，也可通过 Fairy 本身更新。ZIP 或发布版文件夹不会使用启动器自动更新，但可以通过 Customize > Server 中的发布版 ZIP 更新器进行更新。
...
我们在开发 Fairy 时确立了以下几个主要目标：

1. **默认简单，按需强大。** Fairy 的默认体验力求易于理解、直观好用，大多数复杂设置均隐藏在默认工作区之外。我们遵循精心筛选的人机界面指南（HIG）设置合理默认值，让你专注于主聊天窗口。额外的复杂功能与配置不会出现在默认视图中。我们的图形化外壳最能体现这一理念：平时不打扰你，需要时才呈现相应功能。
2. **专注于角色扮演和故事创作。** 相较于上游 Fairy，Fairy 的定位更加明确。我们的目标与模型创意写作领域高度契合，本分支的总体方向也围绕这一使用场景展开。为帮助你以有趣的方式开始使用 LLM 进行创意写作，我们预先内置了教程、预设、扩展和角色卡。
...
4. **更出色的性能。** Fairy 使用 Bun 作为运行时。与 Node.js 相比，Bun 通常性能更好、启动更快，并且针对现代设备进行了更充分的优化，能效也更高。我们仍然支持 Node.js，以提供冗余方案并确保兼容性。
5. **上游兼容性。** 我们尽可能与上游 Fairy 保持向后兼容，并利用其坚实的后端成果，让用户能够轻松从上游切换和迁移。此外，我们希望所有新功能都能适用于各种规模的模型，而不仅限于最前沿的先进模型。
...
Fairy 默认内置了一些额外内容，帮助你立即开始创意写作：

- 详尽的 Fairy 教程，以及帮助你开始 LLM 角色扮演的通用指南。
- 用户界面指南。
...
在大多数受支持的客户端上，Fairy 使用 Bun 而非 Node.js 启动，可显著缩短启动时间、提升整体性能并延长电池续航。对于无法正常支持 Bun 的客户端，我们也支持 Node.js，同时仍可享受其他通用改进。

### 聊天内智能体

Fairy 完整支持以现代模型强大智能体能力为基础设计的智能体工作流。该系统直接接入角色卡，并可完全按照你的需求和偏好自定义。你可以将智能体理解为交给其他模型执行的额外任务：它们伴随主模型的生成流程运行，能以不同方式增强或修改最终输出。
...
Fairy 是 Fairy 的一个分支。Fairy 的绝大多数行为、数据格式及生态系统相关知识仍然适用，我们也会尽可能维持上游兼容性。请在此报告 Fairy 特有的问题；与 Fairy 相关的问题则请向上游报告。
