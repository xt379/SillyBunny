...
This ledger tracks intentional Fairy divergence in upstream-origin files. Its purpose is to keep fork behavior mechanically protected during upstream syncs.

## Rules
- Add or update an entry whenever an upstream-origin file is modified for Fairy behavior.
- Keep entries concise and test-backed.
- Do not use the ledger to justify broad inline fork logic. Prefer moving behavior behind a Fairy module and keeping upstream-origin files as thin adapters.
...
| Divergence reason | Why Fairy must differ from upstream here. |
| Target seam | The Fairy module that should own this behavior, or `none yet` if the seam still needs to be created. |
...
| --- | --- |
| Area | Chat lifecycle. |
| Divergence reason | Fairy adds mobile batching, bottom pinning, manual-scroll suppression, long-chat anchoring, streaming DOM throttles, and issue `#167` scroll stability behavior. |
...
| Divergence reason | Fairy generation flow needs explicit UI lock, stop, agent-generation, and unblock decisions while preserving provider calls, prompt assembly, token accounting, and persistence in the existing generation path. |
...
| Divergence reason | Fairy must reject stale cross-device chat saves instead of allowing last-write-wins overwrites that destroy newer messages. |
...
| Divergence reason | Fairy shell owns top/bottom navigation, chat controls, drawers, mobile actions, search shortcut focus, viewport-reset dispatches, and mirrored connection-profile controls that interact with chat and API state. Its per-device icons-only top bar adds canonical Workspace, Customize, and Characters page clusters, hides redundant shell anchors, deduplicates Quick Access targets, and keeps trailing controls fixed while the page rail pans. |
...
| --- | --- |
| Area | Mobile shell. |
| Divergence reason | Fairy must restore scroll and avoid re-pinning the root while mobile keyboard close/reset events are still settling. |
...
| Divergence reason | Fairy exposes a tool-call recursion limit control that must persist and drive the actual runtime cap instead of showing the browser range midpoint. |
...
| Divergence reason | Fairy needs iOS WebKit conservative streaming and optional smooth-streaming bypass behavior. |
...
| --- | --- |
| Area | Extension boot. |
| Divergence reason | Fairy extension boot needs duplicate manifest protection, deterministic activation ordering, dependency/module gating, disabled dependency handling, and client-version checks while preserving the existing extension runtime loading hooks. |
...
| Divergence reason | Fairy needs duplicate Quick Reply set names from saved/imported data to resolve to one canonical active set so buttons, auto-execute, API lookup, settings selectors, and persisted chat/character links do not duplicate or disappear. |
...
| Divergence reason | Fairy exposes OOC and raw HTML retention depth where `0` must keep the active turn while stripping older context messages. |
...
| Divergence reason | Fairy persists text-generation sampler visibility and manual priority while guarding startup from slow IndexedDB reads that could otherwise overwrite saved selections with fallback state. |
...
| Divergence reason | Fairy keeps `script.js` loaded through its canonical URL, keeps OOC/HTML retention settings copy synchronized with active-turn depth behavior, and exposes core background transparency sliders without requiring Moonlit Echoes. `script.js` must NEVER carry a `?v=` query: every module imports `../script.js` bare, and a versioned tag URL splits ES-module identity so `script.js` evaluates twice and registers every delegated handler twice (all inline-drawer toggles break). Stale-cache protection comes from `src/middleware/frontend-assets.js` serving JS with `Cache-Control: no-cache`, not from URL versioning. |
...
| Divergence reason | Fairy keeps upstream `mobile-styles.css` and the fork mobile shell stylesheet gated to `max-width: 768px` so compact desktop widths use desktop chrome while still receiving upstream 1000px layout rules. |
...
| --- | --- |
| Area | Settings and frontend boot. |
| Divergence reason | Fairy background-image opacity and blur must work without the Moonlit Echoes extension enabled. |
...
| Divergence reason | Fairy keeps horizontal labeled menu rails as the default, supports vertical rail mode without mixing Workspace and Customize shortcuts, and routes Character Menu controls through the canonical drawer when duplicate runtime nodes exist. In icons-only mode, complete character page controls replace the generic drawer-only interaction, and the Characters anchor explicitly opens and highlights only the base Characters page. |
...
| Divergence reason | Fairy must avoid Select2 keyboard-only behavior on touch/mobile OpenRouter/API model and provider selects while preserving the underlying native select values and existing change handlers. |
...
| Divergence reason | Fairy Prompt Manager needs explicit render gating, generation-active waiting, dry-run/live render selection, and scroll restoration while keeping prompt assembly, token counting, and DOM rendering in the existing class. |
...
| Divergence reason | Upstream Prompt Manager cannot identify exact post-macro In-Chat Agent prompts and retained notes after they merge into runtime messages. Fairy needs an inspection-only Agents row with roles and token counts that never enters presets or outbound API payloads. |
...
| Divergence reason | Fairy ships in-chat-agent actions on every chat message (`mes_view_agent_changes`, `mes_fix_trackers`, and PR #446's `mes_run_companions`) in the static message template so the buttons exist before the extension boots. |
...
| Divergence reason | Fairy exposes a fork-only Conversation Mode REST surface that must stay discoverable on both `/api/sillybunny-conversation` and `/api/sillybunny/conversation` without changing upstream Fairy chat/group/settings storage formats. |
...
| Divergence reason | Fairy Conversation REST needs non-streaming access to the existing backend generation handlers without forking prompt assembly or provider-specific request code into a second implementation. |
...
| Divergence reason | Fairy Conversation Mode can narrate DM messages through the existing TTS extension, and Pollinations speech must use the provider's literal audio endpoint instead of the chat-completions wrapper so generated speech matches the requested text. |
...
| Divergence reason | Fairy Conversation selfie actions can fire before Quick Image Gen finishes booting, so the fork needs a small readiness gate rather than duplicating QIG init state in Conversation modules. |
...
| Divergence reason | Fairy Conversation Mode uses a different workspace surface than roleplay chat, so companion handles and welcome-screen recent-chat transitions must stay hidden or visually suppressed while Conversation Mode takes over the shell. |
...
| Divergence reason | Fairy preserves fork-owned account metadata, user directories, and import markers while maintaining upstream archive safety. |
...
| Divergence reason | Fairy integrates World Info with the fork shell and auxiliary lorebook sources and bounds UTF-8 filenames before atomic writes. |
...
| Divergence reason | Fairy adds shell settings, scoped persona behavior, preset synchronization, and character/group Author's Notes while preserving upstream persisted formats. |
...
| Divergence reason | Fairy synchronizes ignored default files, manages fork-owned bundled content, and adds runtime/server safeguards while preserving upstream startup contracts. |
...
| Divergence reason | Fairy adds backup recovery presentation and mobile-aware character/media handling without changing upstream chat or image formats. |
...
| Divergence reason | Fairy carries fork branch policy, branding, Bun-first and Node-compatible launch paths, constrained installs, release automation, and container defaults. |
...
| Divergence reason | Fairy ships fork defaults, agents, presets, and settings while preserving install-time migration and reset behavior. |
...
| Divergence reason | Fairy replaces product identity and translated copy, adds fork provider assets, and keeps browser/PWA boot metadata aligned with the fork. |
...
| Divergence reason | Fairy adapts upstream surfaces to the fork shell, theme tokens, mobile viewport, and WebKit/accessibility requirements. |
...
| Divergence reason | Fairy integrates fork shell, Conversation Mode, provider compatibility, accessibility, persistence safeguards, and prompt behavior through existing upstream browser APIs. |
...
| Divergence reason | Fairy connects built-in extensions to fork agents, profiles, shell behavior, providers, and Conversation Mode without changing extension manifests or persisted settings incompatibly. |
...
| Divergence reason | Fairy adds fork providers, shell/Conversation integrations, hardening, and compatibility behavior while retaining upstream route and storage contracts. |
...
| Divergence reason | Fairy adds runtime parity, fork configuration, security checks, card compatibility, plugin behavior, and provider support around upstream server contracts. |
...
| Divergence reason | Fairy extends inherited coverage for fork routes, direct-app/account-login startup, Node/Bun parity, frontend behavior, and release browser validation. |
